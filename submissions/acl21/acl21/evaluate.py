...
from coreference_model import coreference_model
from nec_metric import evaluate_coreference_nec
from mention_metric import evaluate_mention_identification

def evaluate(results_file, min_speaker_sim_array, max_speaker_merges_array):
    print("loading pretrained coreference model")
    ...
    for name in names:
        print(f"script = {name}\n")
        parsed_file = f"data/coreference/{name}.script_parsed.txt"
        coref_file = f"data/coreference/{name}.coref.mapped.csv"
        ...
        for i, configuration in enumerate(configurations):
            ...
            result = coreference_model(parsed_file, coref_file, coreference_model=predictor, cache_document_to_coref_result=cache_document_to_coref_result, **configuration)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--results", type=str, help="csv file where the results will be saved", default="results/coreference_evaluation.csv")
    parser.add_argument("--min_speaker_similarity", type=float, help="minimum normalized lcs similarity (between 0 and 1) to cluster speaker names", nargs="+", default=[1])
    parser.add_argument("--max_speaker_merges", type=int, help="maximum number of merges to perform for speaker clustering", nargs="+", default=[0])
    args = parser.parse_args()
    results_file = args.results
    min_speaker_sim_array = args.min_speaker_similarity
    max_speaker_merges_array = args.max_speaker_merges
    evaluate(results_file, min_speaker_sim_array, max_speaker_merges_array)
...
def convert_screenplay_and_coreference_annotation_to_json(data_folder, results_folder):
    ...
    #####################################################################
    #### convert character movieparser tags to speakers array
    #####################################################################
    
    movie_without_characters_data = remove_characters(movie_data)

    #####################################################################
    #### convert character-utterance block to add 'says'
    #####################################################################
    ...
    filepath = os.path.join(results_folder, "movie_coref.jsonlines")
    with jsonlines.open(filepath, "w") as writer:
        for movie_coref in movie_data:
            writer.write(movie_coref)

    filepath = os.path.join(results_folder, "movie_without_characters_coref.jsonlines")
    with jsonlines.open(filepath, "w") as writer:
        for movie_coref in movie_without_characters_data:
            writer.write(movie_coref)

    filepath = os.path.join(results_folder, "movie_with_says_coref.jsonlines")
    with jsonlines.open(filepath, "w") as writer:
    ...
    movie_conll_filepath = os.path.join(results_folder, "movie_coref.conll")
    ...
    movie_conll_filepath = os.path.join(results_folder, "movie_without_characters_coref.conll")
    movie_conll.to_csv(movie_conll_filepath, sep="\t", index=False)

    movie_conll = convert_movie_coref_json_to_conll(movie_with_says_data)
    movie_conll_filepath = os.path.join(results_folder, "movie_with_says_coref.conll")
    movie_conll.to_csv(movie_conll_filepath, sep="\t", index=False)
    ...
    filepath = os.path.join(results_folder, "movie_coref.wl.jsonlines")
    with jsonlines.open(filepath, "w") as writer:
        for movie_coref in movie_wl_data:
            writer.write(movie_coref)
    
    movie_wl_data = prepare_for_wlcoref(movie_without_characters_data)
    filepath = os.path.join(results_folder, "movie_without_characters_coref.wl.jsonlines")
    with jsonlines.open(filepath, "w") as writer:
        for movie_coref in movie_wl_data:
            writer.write(movie_coref)

    movie_wl_data = prepare_for_wlcoref(movie_with_says_data)
    filepath = os.path.join(results_folder, "movie_with_says_coref.wl.jsonlines")
    with jsonlines.open(filepath, "w") as writer:
        for movie_coref in movie_wl_data:
            writer.write(movie_coref)

...
flags.DEFINE_string("input_dir", default=os.path.join(data_dir, "movie_coref/results"),
                    help="directory containing subdirectories of preprocessed train and dev script jsonlines")
...
flags.DEFINE_float("warmup", default=-1,
                   help=("number of warmup epochs when the learning rate increases from 0 to max, can be a fraction."
...
flags.DEFINE_integer("patience", default=5,
                     help="maximum number of epochs to wait for dev performance to improve until early-stopping")
...
flags.DEFINE_bool("add_cr_to_coarse", default=True,
                  help="if true, add character scores to the coarse scores for top antecedent selection")
flags.DEFINE_bool("filter_by_cr", default=False,
                  help=("if true, filter antecedents by the predicted character heads for finding predicted "
                        "word-level clusters"))
flags.DEFINE_bool("remove_singleton_cr", default=True,
                  help="if true, remove predicted word-level clusters containing a single predicted character head")

...
# Input and Output
flags.DEFINE_string("input_dir", default=os.path.join(proj_dir, "data/movie_coref/results"), 
    help="Directory of train and dev jsonlines.")
flags.DEFINE_enum("input_type", default="regular", enum_values=["regular", 
    "nocharacters", "addsays"], help="Type of preprocessing applied to screenplays.")
flags.DEFINE_string("weights_file", default=os.path.join(proj_dir, "data/word_level_coref/data/"
        "roberta_(e20_2021.05.02_01.16)_release.pt"), help=("Filepath of word-level coreference "
        "roberta model's weights."))
flags.DEFINE_string("output_dir", default=os.path.join(proj_dir, 
    "data/movie_coref/results/coreference"), help=("Directory to save model weights, predictions, "
    "evaluation metrics, and loss curves."))
flags.DEFINE_string("logs_dir", default=os.path.join(proj_dir, 
    "data/movie_coref/results/coreference/logs"), help="Directory to save logs.")
flags.DEFINE_string("reference_scorer", default=os.path.join(proj_dir, 
    "coref/movie_coref/scorer/v8.01/scorer.pl"), help="Path to conll reference scorer.")
...
flags.DEFINE_integer("train_document_len", default=5120, 
    help="Length of document in words the training screenplay is split into.")
flags.DEFINE_integer("eval_document_len", default=None, help=("Length of document in words the "
    "evaluation screenplay is split into. -1 means the screenplay is not split"))
flags.DEFINE_integer("eval_document_overlap_len", default=512, help=("Number of words overlapping "
    "between successive documents of an evaluation screenplay."))
flags.DEFINE_integer("subword_batch_size", default=2, 
    help="Batch size of subword sequences for encoding.")
...
flags.DEFINE_bool("add_cr_to_coarse", default=False, 
    help="Add character scores to the coarse scores for top antecedent selection.")
flags.DEFINE_bool("filter_by_cr", default=False, help="Filter antecedents by the predicted "
    "character heads for finding predicted word-level clusters.")
flags.DEFINE_bool("remove_singleton_cr", default=False, 
    help="Remove predicted word-level clusters containing a single predicted character head.")
flags.DEFINE_integer("train_cr_epochs", default=0, 
    help="Number of initial epochs during which we only train the character recognition module.")
flags.DEFINE_bool("train_bert_with_cr", default=False, help="Train transformer along with the "
    "character recognition module in the initial train_cr_epochs")

# Model
flags.DEFINE_integer("topk", default=50, 
    help="Maximum number of preceding antecedents to retain after coarse scoring.")
...
# Validators
flags.register_validator("train_document_len", lambda x: x in [2048, 3072, 4096, 5120], 
    "train_document_len should be one of 2048, 3072, 4096, or 5120")
flags.register_validator("eval_document_len", lambda x: x is None or x in [2048, 3072, 4096, 5120], 
    "eval_document_len should be None or one of 2048, 3072, 4096, or 5120")
flags.register_validator("eval_document_overlap_len", lambda x: x in [128, 256, 384, 512], 
    "eval_document_overlap_len should be one of 128, 256, 384, or 512")

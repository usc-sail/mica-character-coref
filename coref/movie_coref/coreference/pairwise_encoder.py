...
class PairwiseEncoder(nn.Module):
    ...
    def forward(self,  
                ...
                speakers: list[str],
                genre: str) -> torch.Tensor:
        word_ids = torch.arange(0, len(top_indices), device=self.device)
        speaker_map = torch.tensor(
            self._speaker_map(speakers), device=self.device)
        ...
        distance = (
            word_ids.unsqueeze(1) - word_ids[top_indices]).clamp_min_(min=1)
        ...
        genre = torch.tensor(
            self.genre2int[genre], device=self.device).expand_as(top_indices)

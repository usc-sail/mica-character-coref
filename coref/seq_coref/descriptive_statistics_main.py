...
The descriptive statistics we find are:
    ...
    4. Total number of mentions and entities when overlaps are removed from an
    entity
"""
...
flags.DEFINE_string("conll_directory", None,
                    "Directory containing conll gold jsonlines files",
                    required=True)
...
def find_descriptive_statistics(language:str, partition:str):
    ...
    mention_pair_relationship_distribution: (
    dict[data.MentionPairRelationship, int]) = {}
    ...
    for document in coref_corpus.documents:
        for cluster in document.clusters:
            ...
            distribution = (
            descriptive_statistics.find_mention_pair_relationships_in_cluster(
                cluster))
            for relationship_type, count in distribution.items():
                if relationship_type not in (
                    ...
                    mention_pair_relationship_distribution[
                        relationship_type] = 0
                mention_pair_relationship_distribution[relationship_type] += (
                count)
            
            max_number = (descriptive_statistics
            .find_maximum_number_of_mentions_covering_a_single_word(cluster))
            maximum_number_of_mentions_covering_some_word = max(
                maximum_number_of_mentions_covering_some_word, max_number)

            non_overlapping_cluster = (descriptive_statistics
            .find_largest_subset_of_non_overlapping_mentions(cluster))
            total_n_mentions_no_overlap += len(non_overlapping_cluster)
            if len(non_overlapping_cluster) > 1:
                total_n_mentions_no_overlap_without_singleton += (
                    len(non_overlapping_cluster))
                total_n_entities_no_overlap_without_singleton += 1
            
    print(f"{language}.{partition}")
    print(f"Total mentions = {total_n_mentions},"
    ...
    for relationship_type, count in (
        mention_pair_relationship_distribution.items()):
    ...
    mention_percentage_decrease_without_singleton = 100*(
        total_n_mentions - total_n_mentions_no_overlap_without_singleton)/(
            total_n_mentions)
    entity_percentage_decrease_without_singleton = 100*(
        total_n_entities - total_n_entities_no_overlap_without_singleton)/(
            total_n_entities)
    print("Filter mentions from cluster s.t. the largest non-overlapping"
          " subset is kept")
    print(f"\tTotal mentions = {total_n_mentions_no_overlap} "
          f"(-{mention_percentage_decrease:.2f}%),"
          f" total entities = {total_n_entities}")

...
def find_mention_pair_relationships_in_cluster(
    ...
    """Find distribution of MentionPairRelationship types in cluster.
    
    If the cluster has n mentions, then there will be nC2 = n(n-1)/2 relationships.
    """
    mention_pair_relationships: list[data.MentionPairRelationship] = []
    cluster = list(cluster)
    for i in range(len(cluster)):
        for j in range(i + 1, len(cluster)):
            mention_pair_relationship = util.find_mention_pair_relationship(cluster[i], cluster[j])
...
def find_maximum_number_of_mentions_covering_a_single_word(cluster: set[data.Mention]) -> int:
    """Find the maximum number of mentions that include the same word"""
...
def find_largest_subset_of_non_overlapping_mentions(
    ...
    """Find largest subset of non-overlapping mentions in cluster"""
    forest = mention_tree.create_mention_forest(cluster)
    mentions: set[data.Mention] = set()
    for mention_node in forest:
        for node in mention_tree.find_largest_subset_of_non_overlapping_mention_nodes(mention_node):
